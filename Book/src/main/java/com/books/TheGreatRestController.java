...
import com.books.cart.*;
import com.books.library.*;
import com.books.signup.*;
...
public class TheGreatRestController
{
	...
	public ResponseEntity<String> fetchCartItems()
	{
		...
		for( Cart c : cdao.getCart() ){
			...
			if( c.getUserid().equals(user) ){
				...
				Library l = ldao.getLibrary(c.getLid());
				
				jobj.put("cid", c.getId());
				
				jobj.put("pname", l.getName());
				jobj.put("pprice", l.getPrice());
				
				jobj.put("pqty",l.getQty());
				
				jobj.put("pimg",l.getImagePath());
	...
	public String deletefromcart(@RequestBody String data)
	{
		...
		try{
			...
			Library l = ldao.getLibrary(c.getId());
